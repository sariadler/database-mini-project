...
## 📥 4. מימוש פיזי, מילוי נתונים וגיבוי המערכת

<details>
<summary><b>💻 א. מימוש פיזי בבסיס הנתונים (DSD)</b></summary>
<br>

בשלב זה הפכנו את המודל הלוגי (ERD) לבסיס נתונים פיזי מתפקד בתוך שרת PostgreSQL.

**תיאור המימוש:**
באמצעות ממשק ה-pgAdmin, הרצנו סקריפטים של SQL ליצירת הסכימה המלאה הכוללת את כל הטבלאות והקשרים. התמונה להלן מציגה את עץ האובייקטים של המערכת, המהווה את ה-DSD (Data Structure Diagram) של הפרויקט:
...
![Tables View](dbFiles/pgAdmin_tables_view.png)

</details>

<details>
<summary><b>🐍 ב. מילוי נתונים (Data Population) - אסטרטגיה וביצוע</b></summary>
<br>
...
* **הלוגיקה**: התחלנו במוצרים, כי במבנה הנתונים שלנו לא ניתן ליצור עיצוב (Design) אם הוא לא משויך למוצר קיים.
...
השתמשנו בסקריפט לייצור נתוני הטבלאות המקשרות הכולל לוגיקה מתקדמת:
* **נתוני JSON**: לכל דגם הוספנו שדה `json_specs` (מפרט טכני) בפורמט JSON, כפי שנדרש בהנחיות.
* **ערכי NULL**: הגדרנו בתוך הלולאה שחלק מהשדות יקבלו ערך **NULL** באופן רנדומלי, כדי לעמוד בדרישה לטיפול בנתונים חסרים.
...
העתקנו את התוכן לתוך ה-Query Tool ב-**pgAdmin** והרצנו אותו. בסיום, ביצענו שאילתת **COUNT** כדי לוודא שכל 500 הרשומות נקלטו בהצלחה בבסיס הנתונים.
...
<details>
<summary><b>💾 ג. גיבוי המערכת (Backup & Restore)</b></summary>
<br>

כדי להבטיח שניתן לבנות את המערכת מחדש מאפס על בסיס קובץ הגיבוי בלבד, ביצענו גיבוי מלא (Full Dump) לבסיס הנתונים דרך ה-pgAdmin.

**תהליך הגיבוי:**
1. **שיטת הגיבוי**: שימוש בכלי ה-Backup המובנה של PostgreSQL בפורמט **Plain**.
2. **הגדרות**: הקובץ נשמר בשם `factory_backup.sql` והוא מכיל את כל מבנה הטבלאות (DDL) ואת כל הנתונים (DML).
3. **יכולת שחזור**: הגיבוי מאפשר שחזור מלא של המערכת בכל מחשב אחר תוך שמירה על כל הישויות והקשרים.
